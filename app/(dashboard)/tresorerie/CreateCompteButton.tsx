...
import { addCompteTresorerie } from './actions'

export default function CreateCompteButton({ dict }: { dict: Dictionary }) {
  ...
  async function handleSubmit(formData: FormData) {
    setLoading(true)
    setError(null)
    const res = await addCompteTresorerie(formData)
    setLoading(false)
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-md bg-surface border border-surface-border px-3 py-2 text-sm font-semibold text-foreground shadow-sm hover:bg-background"
      >
        <Plus className="h-4 w-4" /> {t.newCompteButton}
      </button>
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <h3 className="text-lg font-semibold leading-6 text-foreground mb-4">{t.newCompteTitle}</h3>
                <form action={handleSubmit} id="add-compte-form" className="space-y-4">
                  {error && <p className="text-xs text-danger">{error}</p>}
                  <div>
                    <label className="block text-sm font-medium text-foreground">{c.name}</label>
                    <input name="nom" type="text" required placeholder={t.nameLabelPlaceholder} className="mt-1 block w-full rounded-md bg-background border border-surface-border text-foreground px-3 py-2" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-foreground">{t.typeLabel}</label>
                      <select name="type_compte" className="mt-1 block w-full rounded-md bg-background border border-surface-border text-foreground px-3 py-2">
                        <option value="caisse">{t.typeCaisse}</option>
                        <option value="banque">{t.typeBanque}</option>
                        <option value="mobile_money">{t.typeMobileMoney}</option>
                      </select>
                    </div>
                    <div>

...
const ligneSchema = z.object({
  ...
  prix_unitaire_achat: z.coerce.number().nonnegative('Doit être ≥ 0'),
})
...
export default function CreateAchatButton({ articles, fournisseurs, dict }: { articles: Article[]; fournisseurs: Fournisseur[]; dict: Dictionary }) {
  ...
  const { register, control, handleSubmit, watch, reset, formState: { errors } } = useForm<AchatForm>({
    resolver: zodResolver(achatSchema),
    defaultValues: {
      fournisseur_id: '',
      mode_paiement: 'comptant',
      montant_paye: 0,
      lignes: [{ article_id: '', quantite: 1, prix_unitaire_achat: 0 }],
    },
  })
  ...
  const montantTotal = lignes.reduce((sum, l) => sum + (Number(l.quantite) || 0) * (Number(l.prix_unitaire_achat) || 0), 0)
  ...
  const onSubmit = async (values: AchatForm) => {
    setLoading(true)
    setError(null)
    const res = await creerAchat(

...
export default function ClientRowActions({ client, dict }: { client: Client; dict: Dictionary }) {
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    setLoading(true)
    setError(null)
    const res = await updateClient(client.id, nom, telephone, adresse)
    setLoading(false)
  ...
  async function handleDelete() {
    if (!confirm(t.confirmDelete.replace('{nom}', client.nom))) return
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                ...
                <form onSubmit={handleUpdate} id={`edit-client-${client.id}`} className="space-y-4">

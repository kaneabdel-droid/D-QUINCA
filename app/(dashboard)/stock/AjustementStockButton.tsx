...
export default function AjustementStockButton({ articles }: { articles: Article[] }) {
  ...
  return (
    <>
      ...
      >
        <SlidersHorizontal className="h-4 w-4" /> Ajustement de stock
      </button>
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-surface-border">
              <div className="bg-surface px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <h3 className="text-lg font-semibold leading-6 text-foreground mb-4">Ajustement de stock</h3>
                <form action={handleSubmit} id="ajustement-stock-form" className="space-y-4">
                  {error && <p className="text-xs text-danger">{error}</p>}
                  <div>
                    <label className="block text-sm font-medium text-foreground">Article</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select name="sens" required className="mt-1 block w-full rounded-md bg-background border border-surface-border text-foreground px-3 py-2">
                        <option value="positif">Entrée (+)</option>
                        <option value="negatif">Sortie (-)</option>
                      </select>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input name="motif" type="text" placeholder="Ex: inventaire, casse, erreur de saisie..." className="mt-1 block w-full rounded-md bg-background border border-surface-border text-foreground px-3 py-2" />
                  </div>
                </form>
              </div>

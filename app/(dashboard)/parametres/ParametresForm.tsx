...
export default function ParametresForm({ entreprise }: { entreprise: Entreprise }) {
      ...
      <div className="rounded-xl border border-surface-border bg-surface p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            >
              <Upload className="h-4 w-4" /> {uploading ? 'Envoi...' : logoUrl ? 'Changer le logo' : 'Ajouter un logo'}
            </label>
            <p className="mt-1 text-xs text-foreground-muted">PNG ou JPG, 2 Mo maximum.</p>
          </div>
        </div>
      </div>

      <form action={handleSubmit} className="rounded-xl border border-surface-border bg-surface p-6 space-y-4">
        <h2 className="font-semibold text-foreground mb-2">Informations générales</h2>
        {error && <p className="text-sm text-danger">{error}</p>}

        <div>
          <label className="block text-sm font-medium text-foreground">Dénomination</label>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground">Identification fiscale (NINEA, RCCM ou équivalent)</label>
          <input name="identification" type="text" defaultValue={entreprise.identification} placeholder="Ex : NINEA 0012345678" className="mt-1 block w-full rounded-md bg-background border border-surface-border text-foreground px-3 py-2" />
        </div>

        <div>
          <label className="block text-sm font-medium text-foreground">Devise de travail</label>
          ...
          <p className="mt-1 text-xs text-foreground-muted">Utilisée pour tous les montants affichés et imprimés dans l&apos;application.</p>
        </div>
